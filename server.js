...
Be concise, specific, and use actual names and data from the records. Format responses with markdown for readability.`;
...
app.post('/api/summarize-lead', async (req, res) => {
  try {
    const { lead, interactions } = req.body;
    ...
    const userPrompt = `Summarize this home care lead in 3-5 bullet points for a sales rep. Be very concise — no headers, no sections, just the key facts and one recommended next step.

LEAD: ${lead.name || 'Unknown'}, DOB ${lead.dateOfBirth || '?'}, ${lead.careLevel || lead.careType || '?'} care, stage: ${lead.stage || '?'}, score: ${lead.score || '?'}
Contact: ${lead.contactPerson || ''} (${lead.relationship || ''}), phone: ${lead.contactPhone || ''}, email: ${lead.contactEmail || ''}
Timeline: ${lead.timeline || '?'}, Budget: ${lead.budget || '?'}, Hours: ${lead.hoursPerDay || '?'}, Source: ${lead.source || '?'}
Last Contact: ${lead.lastContactDate || '?'}
${lead.intakeNote ? 'Intake: ' + JSON.stringify(lead.intakeNote) : ''}
Activity (${interactions?.length || 0}): ${interactionsSummary}`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a senior care sales assistant. Generate very short lead summaries as 3-5 bullet points using markdown bold for key terms. Max 80 words total. End with one specific next step.',
          },
          { role: 'user', content: userPrompt },
        ],
        temperature: 0.5,
    ...
    res.status(500).json({ error: 'Failed to summarize lead' });
  }
});
