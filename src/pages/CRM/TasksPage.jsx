...
import { Plus, Trash2, CheckSquare, Eye, EyeOff } from "lucide-react";
...
export default function TasksPage({ leads = [], tasks = [], onAddTask, onUpdateTask, onDeleteTask, alerts = [], onNavigate, setAutoOpenLeadId }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar title="Task List" subtitle={`${pendingTasks.length} pending tasks`} alerts={alerts} action={{ label: "Add Task", onClick: () => setShowAddForm(!showAddForm) }} />
      <div className="flex-1 overflow-auto p-6 space-y-4">
        ...
        {/* Add form */}
        {showAddForm && (
          <form onSubmit={handleSubmitTask} className="p-3 rounded-md border border-dashed border-border bg-muted/30 space-y-2">
            <Input
          ...
          {filteredTasks.length === 0 ? (
            ...
            <div className="space-y-1">
              {filteredTasks.map((task) => {
                ...
                return (
                  <div key={task.id} className="flex items-center gap-2 py-2 px-2 rounded-md hover:bg-muted/50 transition-colors group">
                    <button
        ...
        {doneTasks.length > 0 && (
          <div className="rounded-lg border border-border bg-card p-4 shadow-crm-sm">
            ...
            >
              ...
              <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-muted px-1 text-[10px] font-medium text-muted-foreground">
                {doneTasks.length}
              </span>
            </button>
            {showCompleted && (
              <div className="mt-2 space-y-1">
                {doneTasks.map((task) => {
                  const lead = leadsById[task.lead_id];
                  return (
                    <div key={task.id} className="flex items-center gap-2 py-1.5 px-2 opacity-50 group">
                      <button
                        onClick={() => handleToggleTask(task)}
